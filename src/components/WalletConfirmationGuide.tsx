...
const WalletConfirmationGuide: React.FC<WalletConfirmationGuideProps> = ({
  ...
  return (
    <Paper elevation={2} sx={{ p: 2, mt: 2, mb: 2, maxWidth: "100%" }}>
      <Alert severity="info" sx={{ mb: 2 }} variant="filled">
      ...
      <Box sx={{ ml: 2 }}>
        <Typography paragraph sx={{ display: "flex", alignItems: "center" }}>
          1. 查看浏览器右上角是否有MetaMask扩展图标
          <span style={{ color: "orange", marginLeft: "4px" }}>闪烁</span>
        </Typography>
        ...
        <Typography paragraph>4. 在MetaMask中点击"确认"按钮完成交易</Typography>
      </Box>

      <Alert severity="warning" variant="filled">
        如果反复出现此问题，请尝试: 1)刷新页面 2)重启MetaMask
        3)在MetaMask设置中"重置账户"
      </Alert>
    </Paper>
  )
